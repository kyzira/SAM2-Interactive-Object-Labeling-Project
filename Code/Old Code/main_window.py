...
import math
from sam2_propagate_per_instance import SamInstanceSegmentation 
...
class ImageDisplayWindow(tk.Tk):
    ...
    def activate_splitting_mode(self):
        def on_confirm():
            # Code to execute if the split is confirmed
            # Close the popup and proceed with the split
            popup.destroy()
            self.splitting_mode = True

        def on_cancel():
            # Code to execute if the split is canceled
            # Close the popup without splitting
            popup.destroy()

        # Create popup window
        popup = tk.Toplevel()
        popup.title("Confirm Split")
        popup.geometry("250x180")

        # Label and buttons in popup
        label = tk.Label(popup, text="Do you want to split the object?\n\n If Yes:\n 1.Click Start of Instance\n 2.Click End of Instance")
        label.pack(pady=10)
        ...
        popup.transient(self)  # Make the popup appear above the main window
    ...
    def split_object(self, clicked_frame_index):       
        if self.first_index == None:
            self.first_index = clicked_frame_index
            return
        
        start_index = self.first_index
        end_index = clicked_frame_index
        ...
        # Get the Frame Numbers of those Frames
        start_frame = int(self.frame_info.get_frame_name_list()[start_index].split(".")[0])
        end_frame = int(self.frame_info.get_frame_name_list()[end_index].split(".")[0])

        print(f"Start Frame: {start_frame}, End Frame: {end_frame}")

        # Check if Tracking Splits is in Json
        ...
        if len(tracking_splits[sel_obj]) > 0:
            for a, b in tracking_splits[sel_obj]:
                if a < start_frame < b or a < end_frame < b:
                    popup = tk.Toplevel(self)  # Attach to the main root window if available
                    popup.title("Error")
                    popup.geometry("300x100")

                    # Label and buttons in popup
                    label = tk.Label(popup, text="Error: Areas Overlap!\n\n Close this pop-up to Abort")
                    label.pack(pady=10)
        ...
        tracking_splits[sel_obj].append((start_frame, end_frame))
        tracking_splits[sel_obj].sort(key=lambda x: x[0])

        self.json.add_to_info(key="Tracking Splits", value=tracking_splits)
    ...
    def track_object(self):
        ...
        sam_instance_segmentation = SamInstanceSegmentation(sam_model=self.sam_model,
                                                            frame_dir=self.frame_info.get_frame_dir(),
                                                            frame_name_list=self.frame_info.get_frame_name_list(),
                                                            json_data=self.json.get_json(),
                                                            observation=self.grid.radio_var.get())
         
        sam_instance_segmentation.start_process_for_all_instances()
        video_segments = sam_instance_segmentation.get_predictions()
